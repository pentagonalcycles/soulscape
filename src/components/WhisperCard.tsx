...
const reactionConfigs: ReactionConfig[] = [
  { type: "understanding", label: "I feel this", icon: "🤍", color: "#9d7cd8" },
  { type: "hope", label: "This gave me hope", icon: "✨", color: "#f5d062" },
  { type: "company", label: "You are not alone", icon: "🫂", color: "#2dd4a8" },
  { type: "less_alone", label: "Less alone", icon: "🌌", color: "#60a5fa" },
  { type: "comfort", label: "Send warmth", icon: "💫", color: "#e879a8" },
];
...
export default function WhisperCard({
  ...
  return (
    <motion.article
      className="rounded-2xl sanctuary-glass-card p-5 sm:p-6 relative group"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.6, ease: [0.25, 0.46, 0.45, 0.94] }}
      whileHover={{
        boxShadow: "0 0 30px rgba(157, 124, 216, 0.08)",
      }}
    >
      ...
      {hasContentWarning && isHidden && (
        <div className="absolute inset-0 rounded-2xl bg-elovayne-void/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-sm text-elovayne-muted font-body mb-3">
            This whisper may contain sensitive content
          </p>
          <button
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-elovayne-violet/15 flex items-center justify-center flex-shrink-0">
          ...
          <div>
            ...
            <p className="text-elovayne-dim/60 text-xs font-body">
              ...
              >
                  ...
                  <>
                    {confirmDelete ? (
                      <div className="px-3 py-2">
                        <p className="text-xs text-elovayne-muted mb-2 font-body">Dissolve this whisper?</p>
                ...
                {showReportReason ? (
                  ...
                  <>
                    ...
                    <button
                      onClick={() => {
                        navigator.clipboard?.writeText(`${window.location.origin}/sanctuary#${id}`);
                        setShowMenu(false);
                      }}
                      className="w-full text-left px-3 py-2 text-sm text-elovayne-muted hover:text-elovayne-light hover:bg-elovayne-violet/10 rounded-lg transition-colors font-body"
                    >
      ...
      <div className="mb-4 pl-11">
        <p className="text-elovayne-light font-body text-sm leading-relaxed italic">
          &ldquo;{content}&rdquo;
        </p>
      </div>
      ...
      <AnimatePresence>
        {showReplies && (
          ...
          >
            <div className="mt-4 pl-11 space-y-3 border-l border-elovayne-violet/10 ml-4">
              {replies.map((reply) => (
                <div key={reply.id} className="pl-4 py-2">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs">{reply.isAnonymous ? "🌙" : "✦"}</span>
                    <p className="text-xs text-elovayne-light font-body">
                      {reply.isAnonymous ? "Anonymous" : reply.displayName || "Someone"}
                    </p>
                    <span className="text-xs text-elovayne-dim/50 font-body">
                      {getTimeAgo(reply.createdAt)}
                    </span>
                  </div>
                  <p className="text-xs text-elovayne-muted font-body leading-relaxed pl-5">
                    {reply.content}
                  </p>
                </div>
              ))}
              ...
              <div className="pl-4 py-2">
                <p className="text-xs text-elovayne-dim/50 font-body mb-2 italic">
                  Respond with kindness. There is a person behind every whisper.
                </p>
                <div className="flex gap-2">
                  <input
                    type="text"
                    ...
                    className="flex-1 bg-elovayne-void/30 border border-elovayne-violet/15 rounded-lg px-3 py-2 text-xs text-elovayne-light placeholder-elovayne-dim/50 focus:outline-none focus:border-elovayne-violet/30 transition-colors font-body"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-full text-xs font-body text-elovayne-light bg-elovayne-violet/30 hover:bg-elovayne-violet/40 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
