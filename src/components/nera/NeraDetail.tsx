      ...
      <div className="max-w-2xl mx-auto px-5 sm:px-8 pt-24 pb-20">
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-4">
            <span className="inline-flex items-center gap-1.5 text-[11px] px-2.5 py-1 rounded-full font-medium" style={{ background: `${neraType.color}0A`, color: neraType.color, border: `1px solid ${neraType.color}18` }}>
          ...
          <h1
            ...
            style={{ fontWeight: 400, color: "var(--text-primary, #0f172a)", fontFamily: "var(--font-heading)", letterSpacing: "0.01em", lineHeight: 1.2 }}
          >
            {nera.title}
          </h1>

          {/* Host */}
          <p className="text-[13px] mb-4" style={{ color: "var(--text-muted, #64748b)" }}>
            Hosted by <span className="font-medium" style={{ color: "var(--text-primary, #0f172a)" }}>{nera.host_name}</span>
          </p>

          {/* Meta */}
          <div className="flex flex-wrap gap-x-5 gap-y-2 mb-5">
            <div className="flex items-center gap-2">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--text-dim, #94a3b8)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" />
              </svg>
              <span className="text-[13px]" style={{ color: "var(--text-muted, #64748b)" }}>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[13px] font-medium" style={{ color: "var(--text-primary, #0f172a)" }}>Attendees</h3>
            <span className="text-[12px]" style={{ color: isFull ? "#ef4444" : "var(--text-dim, #94a3b8)" }}>
              {nera.current_participants}/{nera.max_participants}
            </span>
          </div>
          {attendees.length === 0 ? (
            <p className="text-[13px]" style={{ color: "var(--text-dim, #94a3b8)" }}>No one has joined yet.</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {attendees.map((a, i) => (
                <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-full" style={{ background: `${neraType.color}08`, border: `1px solid ${neraType.color}12` }}>
                  ...
                  <span className="text-[12px] font-medium" style={{ color: "var(--text-primary, #0f172a)" }}>{a.name}</span>
                </div>
              ))}
            </div>
          )}
                ...
                <div className="max-h-56 overflow-y-auto mb-4 space-y-3 pr-1" style={{ scrollbarWidth: "thin" }}>
                  {messages.map((m) => (
                    <div key={m.id} className="text-[13px]">
                      <span className="font-medium" style={{ color: "var(--text-primary, #0f172a)" }}>{m.author_name}: </span>
                      <span style={{ color: "var(--text-muted, #64748b)" }}>{m.content}</span>
                    </div>
                  ))}
                  {messages.length === 0 && (
                    <p className="text-[12px]" style={{ color: "var(--text-dim, #94a3b8)" }}>No messages yet. Say something!</p>
                  )}
        ...
        {isHost && nera.status === "upcoming" && (
          ...
          >
            <h3 className="text-[13px] font-medium mb-3" style={{ color: "var(--text-primary, #0f172a)" }}>Host Controls</h3>
        ...
        >
          ...
          <ul className="text-[12px] space-y-1.5" style={{ color: "var(--text-muted, #64748b)", lineHeight: 1.6 }}>
        ...
        {!isHost && !showReport && !reported && (
          <button onClick={() => setShowReport(true)} className="text-[12px]" style={{ color: "var(--text-dim, #94a3b8)" }}>
            Report this Nera
          </button>
        )}
        {showReport && !reported && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
          >
            <p className="text-[13px] font-medium mb-3" style={{ color: "var(--text-primary, #0f172a)" }}>Why are you reporting?</p>
